...
// 24 questions: 6 per category (phishing, password, qr, scam),
// 2 easy + 2 medium + 2 hard per category (8/8/8 overall).
// Judgment questions use a 3-way scale:
//   "Legit — proceed" / "Suspicious — verify via official channel" / "Scam — delete & report"
// so the answer isn't always the extreme option.
const questions = [
  // ---------------- PHISHING (6) ----------------
  {
    category: 'phishing',
...
async function seed() {
  ...
  for (const q of questions) {
    counts[q.category] = counts[q.category] || { easy: 0, medium: 0, hard: 0 };
    counts[q.category][q.difficulty] += 1;
  }
  console.log(`Seeded ${questions.length} questions:`, JSON.stringify(counts));
  await disconnect();
}
